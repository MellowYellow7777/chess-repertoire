class ContextMenu {
  ...
  static create(x,y) {
    ...
    this.current = new ContextMenu(x,y);
    return this.current;
  }
...
document.onmousedown =  function(event) {
  var menu = ContextMenu.current;
  if (!menu) return;
  if (!menu.element.contains(event.target)) ContextMenu.destroy();
}

function createContextMenu(event) {
  var menu = ContextMenu.create(event.clientX, event.clientY);
  menu.addMenuItem('Look Up "this.x"');
  menu.addLine();
  menu.addMenuItem('Search With Google');
  menu.addLine();
  ...
  menu.addLine();
  menu.addMenuItem('Font');
  menu.addMenuItem('Spelling and Grammar');
  menu.addMenuItem('Substitutions');
  menu.addMenuItem('Transformations');
  menu.addMenuItem('Speech');
  menu.addMenuItem('Layout Orientation');
}
...
function createFileContextMenu(x,y) {
  ...
  menu.addMenuItem('Open', () => {
    ...
    var _saves = saves;
    saves = loadSaveData();
    new FloatingItemListWindow('Open',saves);
  });
  menu.addMenuItem('Examples', () => {
    menu.destroy();
    new FloatingItemListWindow('Open',examples);
  });
  menu.addMenuItem('DIL Library', () => {
    menu.destroy();
    new FloatingItemListWindow('Open',DILLibrary);
  });
...
function createEditContextMenu(x,y) {
  ...
  menu.addMenuItem('Global Settings', () => {
    menu.destroy();
    FloatingWindow2.addSettingsDialogue();
  });
...
function createDevContextMenu(x,y) {
  if (contextMenu) contextMenu.destroy();
  contextMenu = new ContextMenu(x,y);
  var menu = contextMenu;
  ...
  menu.element.className = 'context-menu-dropdown';
    ...
    menu.addToggleMenuItem(
      () => drawComponentIndex,
      'Hide Component Indicies',
      'Show Component Indicies',
      () => drawComponentIndex = !drawComponentIndex,
    );
